...
def get_seq_record(record, start, stop, description):
    ...
    if first_position and last_position:
        ...
        if not record.is_reverse:
            seq = Seq(
                record.get_forward_sequence()[first_position[0] : last_position[0]],
            ...
            seq = Seq(
                record.get_forward_sequence()[
                    length - last_position[0] : length - first_position[0]
                ],
                IUPAC.IUPACUnambiguousDNA(),
            )
            qual = list(record.get_forward_qualities())[
        ...
        rec = SeqRecord(
            seq,
            id=name,
            description=("|").join([description, direction[record.is_reverse]]),
        )
...
def main(arguments):
    ...
    with pysam.AlignmentFile(args.inbam, "rb") as samfile:
        for record in samfile.fetch(
            contig=args.contig, start=args.start, stop=args.stop
        ):
            my_records.append(
                get_seq_record(record, args.start, args.stop, description)
            )
