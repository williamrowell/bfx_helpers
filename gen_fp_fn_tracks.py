...
def filter_vcf(invcf):
    ...
    for record in vcf_reader:
        if (
            record.genotype("QUERY")["BVT"] == "INDEL"
            and record.genotype("QUERY")["BD"] == "FP"
        ):
            vcf_writer_FP_INDEL.write_record(record)
        elif (
            record.genotype("TRUTH")["BVT"] == "INDEL"
            and record.genotype("TRUTH")["BD"] == "FN"
        ):
            vcf_writer_FN_INDEL.write_record(record)
        elif (
            record.genotype("QUERY")["BVT"] == "SNP"
            and record.genotype("QUERY")["BD"] == "FP"
        ):
            vcf_writer_FP_SNP.write_record(record)
        elif (
            record.genotype("TRUTH")["BVT"] == "SNP"
            and record.genotype("TRUTH")["BD"] == "FN"
        ):
