...
def length_match(curr, prev, len_wiggle):
    ...
    return (
        abs(curr.infer_query_length() - prev.infer_query_length())
...
def flush_buffer(curr, buffer, aln_wiggle):
    ...
    to_write = list()
    while buffer and not (
        ref_match(curr, buffer[0]) and start_match(curr, buffer[0], aln_wiggle)
    ):
...
def write_stats(duplicate_set, total_set, args):
    ...
    values = [
        ...
        (
            "PCR duplicate frequency",
            "{:.04f}%".format(100 * len(duplicate_set) / float(len(total_set))),
        ),
    ]
    if args.stats:
...
def main(arguments):
    ...
    with pysam.AlignmentFile(args.inbam, "rb") as infile, pysam.AlignmentFile(
        args.outbam, "wb", template=infile
    ) as outfile:
        ...
        for curr in infile:
            ...
            if curr.query_name in duplicate_set or is_duplicate(
                curr, buffer, args.aln_wiggle, args.len_wiggle
            ):
