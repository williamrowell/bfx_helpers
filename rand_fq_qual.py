...
def main(arguments):
    ...
    parser.add_argument('fastq', help="file to process",
        type=argparse.FileType('r'))
    parser.add_argument('-s', '--seed', help="random seed (23)", type=int,
        default=23)
    parser.add_argument('-f', '--fixed_score',
    ...
    for record in SeqIO.parse(args.fastq, 'fastq'):
        if args.fixed_score:
            record.letter_annotations['phred_quality'] = \
                [args.fixed_score] * len(record.letter_annotations['phred_quality'])
        else:
